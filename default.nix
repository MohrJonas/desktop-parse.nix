lib: let
  ...
  parseDesktopFile = content: let
    ...
    elements =
      ...
      (builtins.map
        (line:
          if util.isSectionHeading line
          then util.trimSectionHeading line
          else if util.isEmptyLine line
          then null
          else if util.isContentLine line
          then util.parseContentLine line
          else throw "Unknown line type ${line}")
        lines);
  in
    (builtins.foldl'
      (state: element:
